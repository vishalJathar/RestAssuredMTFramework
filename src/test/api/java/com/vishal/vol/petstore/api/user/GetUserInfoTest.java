...
public class GetUserInfoTest extends AbstractCommonOperations implements ApiTestWithoutPayload {
	...
	@ParameterizedTest(name = "Positive Test Case: [id: {0}, username: {1}, http_status: {2}, request_payload_file_name: {3}, response_schema_file_name: {4}, response_exact_matcher_file_name: {5}]")
	...
	@ParameterizedTest(name = "Negative Test Case: [id: {0}, username: {1}, http_status: {2}, request_payload_file_name: {3}, request_payload_file_name: {4}, code: {5}, sub_code: {6}]")
	@CsvFileSource(resources = "/" + COMPLETE_RESOURCE_PATH
	...
	public void enhanceRequest(RequestSpecification request, ApiTestRequestBase requestBase) {
		...
		request.pathParam("username", requestBase.getEntityName());
